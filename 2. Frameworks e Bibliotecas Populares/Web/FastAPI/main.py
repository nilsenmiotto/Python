...
def retorna_produtos(categoria: Categoria = None, skip: int = 0, limit: int = 10):
    ...
    if categoria is None:
        ...
        filtro_por_categoria = list(
            filter(
                lambda produto: produto["categoria"] == categoria.value, produtos_data
            )
        )
...
def retorna_produto(id: int):
    ...
    dados = next(
        (produto for produto in produtos_data if produto.get("id") == id), None
    )
...
def cadastra_produto(produto: Produto_NEW):
    ...
    for encontrado in produtos_data:
        if (
            encontrado.get("nome") == produto.nome
            and encontrado.get("categoria") == produto.categoria.value
        ):
